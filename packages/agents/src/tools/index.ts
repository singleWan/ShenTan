...
import { z } from 'zod';
import {
  webSearch as doSearch,
  searchSocialMedia as doSearchSocialMedia,
  getDefaultSearchManager,
  type SearchResult,
} from '@shentan/crawler';
...
export function createTools(db: Database) {
  ...
  const webSearch = tool({
    description:
      '搜索互联网获取信息。支持多种搜索模式和丰富的过滤选项。返回搜索结果列表（标题、URL、摘要、发布日期）。deep 模式支持关联性早停：提供 relevantKeywords 后会逐页评估关联度，关联太弱时自动停止翻页。',
    inputSchema: z.object({
      ...
      categories: z
        .array(z.string())
        .optional()
        .describe('搜索分类，如 ["social media"] 用于搜索社交媒体内容'),
      engines: z
        .array(z.string())
        .optional()
        .describe('指定搜索引擎，如 ["google", "bing", "baidu"]'),
      timeRange: z
        .enum(['day', 'week', 'month', 'year'])
        .optional()
        .describe(
          '时间范围过滤：day=最近一天，week=最近一周，month=最近一月，year=最近一年。用于搜索最新信息时设置。',
        ),
      searchMode: z
      ...
      relevantKeywords: z
        ...
        .optional()
        .describe(
          '关联性关键词列表，用于 deep 模式的逐页关联性评估。从搜索目标中提取的2-4个核心关键词。搜索引擎会检查每页结果是否包含这些关键词，如果连续2页关联度低于15%则自动停止翻页，避免在无关方向上浪费搜索深度。',
        ),
    }),
    execute: async ({
      query,
        ...
        switch (searchMode) {
          case 'deep':
            ...
            return formatResults(
              await manager.searchWithPages(query, {
                ...searchOptions,
                pageCount: 5,
                relevantKeywords,
              }),
          ...
          case 'broad':
            // 多分类并行搜索
            return formatResults(
              await manager.searchMultiCategory(query, searchOptions),
              maxResults ?? 30,
            );

          case 'news':
            return formatResults(
              await manager.search(query, { ...searchOptions, categories: ['news'] }),
              maxResults ?? 15,
            );
          ...
          case 'social':
            ...
            return formatResults(
              await manager.search(query, { ...searchOptions, categories: ['social media'] }),
              maxResults ?? 15,
            );
  ...
  const scrapePage = tool({
    description:
      '爬取指定URL的网页内容。返回页面标题、正文文本和页面图片URL（如果有）。用于获取搜索结果中的详细内容。',
    inputSchema: z.object({
  ...
  const saveEvents = tool({
    description:
      '保存收集到的事件到数据库。支持所有事件类型，包括生平事件、发言、政策、声明等。每个事件包含标题、描述、日期、分类、重要度等信息。发言/政策类事件应额外填写 content（完整文本）、platform（来源平台）、authorHandle（发言账号）。',
    inputSchema: z.object({
        ...
        .array(
          z.object({
            ...
            dateSortable: z
              ...
              .optional()
              .describe(
                '可排序日期。历史人物用 YYYY-MM-DD（如 1946-06-14）；虚构角色用 FIC- 前缀（章节用 FIC-CH0025、阶段用 FIC-PH001、叙事序号用 FIC-SEQ0003）',
              ),
            category: z.enum(EVENT_CATEGORIES).optional().describe('事件分类'),
            content: z
              .string()
              .optional()
              .describe('发言/政策/声明的完整文本内容（仅 speech/policy/statement 类事件需要）'),
            platform: z
              .string()
              .optional()
              .describe(
                '来源平台（如 twitter/weibo/facebook/instagram/youtube/zhihu/news/official/other）',
              ),
      ...
      reactions: z
        .array(
          z.object({
            ...
            reactorType: z
              .enum(['person', 'organization', 'country', 'media', 'group'])
              .describe('反应方类型'),
  ...
  const getEvents = tool({
    description:
      '获取角色已有的事件列表。用于查看已收集的事件，决定下一步拓展方向。支持按分类和重要度过滤。',
    inputSchema: z.object({
      characterId: z.number().describe('角色ID'),
      ...
      category: z
        ...
        .optional()
        .describe(
          '按分类过滤（life/career/political/conflict/achievement/scandal/speech/policy/statement/rumor/other）',
        ),
    }),
    execute: async ({ characterId, minImportance, category }) => {
      ...
      const formatted = events
        .map((e) => {
          const extra = e.content
            ? ` [${e.category}${e.platform ? '/' + e.platform : ''}]`
            : ` [${e.category}]`;
  ...
  const getReactions = tool({
    ...
    execute: async ({ eventId }) => {
      ...
      const formatted = reactions
        .map(
          (r) =>
            `- [${r.reactorType}] ${r.reactor}: ${r.reactionText ?? '(无内容)'} (${r.sentiment ?? '未知态度'})`,
        )
        .join('\n');
  ...
  const updateCharacter = tool({
    description:
      '更新角色信息（描述、状态、图片URL）。imageUrl 应该是从权威来源（如维基百科、官方页面）爬取到的角色海报或肖像图片的 URL。系统会自动下载图片到本地。优先选择高质量、正面、清晰的图片。',
    inputSchema: z.object({
      characterId: z.number().describe('角色ID'),
      ...
      imageUrl: z
        .string()
        .optional()
        .describe('角色图片URL（海报、肖像等），从爬取页面的 og:image 或其他来源获取'),
    }),
      ...
      relations: z
        .array(
          z.object({
            ...
            relationType: z
              .enum(['ally', 'enemy', 'family', 'colleague', 'rival', 'mentor', 'friend', 'other'])
              .describe('关系类型'),
