...
export async function resolveAliases(
  ...
  try {
    const result = await withRetry(
      ...
      (attempt, err, delay) => {
        createLogger({ category: 'alias-resolver' }).warn(
          `第 ${attempt} 次重试（${delay}ms 后）: ${err.message}`,
        );
      },
    );
    ...
    createLogger({ category: 'alias-resolver' }).warn(
      `generateObject 失败，降级到 generateText: ${(objErr as Error).message}`,
    );
  }
...
export function formatAliasesForPrompt(aliases: CharacterAlias[]): string {
  ...
  for (const [lang, group] of Object.entries(byLanguage)) {
    ...
    const others = group
      .filter((a) => a.type !== 'formal' && a.type !== 'nickname')
      .map((a) => `${a.name}${a.usageContext ? `(${a.usageContext})` : ''}`);
