...
export type RelationType = 'ally' | 'enemy' | 'family' | 'colleague' | 'rival' | 'mentor' | 'friend' | 'other';

export async function saveRelations(db: Database, input: {
  characterId: number;
  relations: Array<{
    targetName: string;
  ...
  const targetChars = await db.select({ id: characters.id, name: characters.name })
    .from(characters).where(inArray(characters.name, targetNames));
  ...
  for (const rel of input.relations) {
    ...
    if (existingSet.has(`${targetId}:${rel.relationType}`)) { skipped++; continue; }
...
export async function getCharacterRelations(db: Database, characterId: number): Promise<CharacterRelation[]> {
  ...
  // 批量查询角色名称
  const charIds = [...new Set(uniqueRelations.flatMap(r => [r.fromCharacterId, r.toCharacterId]))];
